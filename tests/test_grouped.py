...
class TestStructRows:
    def test_names_and_rows(self) -> None:
        names, rows, cat = _struct_rows(_struct([{"a": 1.0, "b": 2.0}, {"a": 3.0, "b": 4.0}]))
        assert names == ["a", "b"]
    ...
    def test_string_field_is_categorical(self) -> None:
        s = pa.array(
            [{"city": "NYC", "x": 1.0}, {"city": "LA", "x": 2.0}],
            type=pa.struct([pa.field("city", pa.string()), pa.field("x", pa.float64())]),
        )
        names, rows, cat = _struct_rows(s)
        assert names == ["city", "x"]
        assert cat == [True, False]
        assert rows == [["NYC", 1.0], ["LA", 2.0]]
...
class TestMatrixFor:
    def test_aligns_by_name_not_position(self) -> None:
        ...
        m = _matrix_for(["a", "b"], s, [False, False])
        assert np.array_equal(m, np.array([[10.0, 20.0]]))

    def test_categorical_kept_as_string(self) -> None:
        s = pa.array(
            [{"city": "NYC", "x": 1.0}],
            type=pa.struct([pa.field("city", pa.string()), pa.field("x", pa.float64())]),
        )
        m = _matrix_for(["city", "x"], s, [True, False])
    ...
    def test_extra_columns_ignored(self) -> None:
        ...
        assert np.array_equal(_matrix_for(["a", "b"], s, [False, False]), np.array([[1.0, 2.0]]))

    def test_missing_feature_errors(self) -> None:
        with pytest.raises(ValueError, match="missing model column"):
            _matrix_for(["a", "b", "z"], _struct([{"a": 1.0, "b": 2.0}]), [False, False, False])
